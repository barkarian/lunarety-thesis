...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.9\textwidth}{\centering\vspace{1cm}\textit{[ΔΙΑΓΡΑΜΜΑ: PayloadCMS Architecture]}\\\small Περιγραφή: Διάγραμμα που δείχνει το PayloadCMS στο κέντρο, με βέλη προς τα εξής στοιχεία: ORM (Drizzle), Lifecycles (Hooks), Admin UI, REST API, GraphQL API, Local API. Το διάγραμμα υπογραμμίζει πώς ένα μόνο CollectionConfig ορίζει όλα αυτά τα στοιχεία ταυτόχρονα, δείχνοντας την ενότητα και την αυτοματοποίηση που προσφέρει το PayloadCMS.\vspace{1cm}}}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.9\textwidth}{\centering\vspace{1cm}\textit{[ΔΙΑΓΡΑΜΜΑ: OpenAPI/Swagger Code Generation Flow]}\\\small Περιγραφή: Διάγραμμα που δείχνει τη ροή από το API Documentation προς την αυτόματη παραγωγή Client. Στο αριστερό μέρος: «API Endpoints» → «OpenAPI/Swagger Documentation» (παραγόμενο από \code{next-swagger-doc}). Στο κέντρο: «OpenAPI Specification (YAML/JSON)». Στο δεξί μέρος: «TypeScript Client SDK» (παραγόμενο από \code{openapi-typescript-codegen}) → «Website Project». Βέλη δείχνουν τη ροή: API → Documentation → Specification → Client Generation → Website. Ετικέτες: «Automatic Documentation», «Type-Safe Client», «Synchronized API». Το διάγραμμα υπογραμμίζει πώς ένα OpenAPI specification μπορεί να χρησιμοποιηθεί για την αυτόματη παραγωγή ενός πλήρως τυποποιημένου client, εξαλείφοντας manual integration work.\vspace{1cm}}}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.9\textwidth}{\centering\vspace{1cm}\textit{[ΔΙΑΓΡΑΜΜΑ: OpenRouter Architecture]}\\\small Περιγραφή: Διάγραμμα που δείχνει το OpenRouter στο κέντρο, με βέλη προς πολλαπλά LLM μοντέλα (GPT-4, Claude 3.5 Sonnet, Llama 3, Mistral, κ.α.) στη μία πλευρά και βέλη προς την Πλατφόρμα Lunarety V2 στην άλλη. Ετικέτες: «Unified API», «Model Selection», «Single Integration Point». Το διάγραμμα υπογραμμίζει πώς το OpenRouter λειτουργεί ως gateway που επιτρέπει πρόσβαση σε πληθώρα μοντέλων μέσω ενός ενιαίου API, εξαλείφοντας την ανάγκη για πολλαπλές ενσωματώσεις.\vspace{1cm}}}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.9\textwidth}{\centering\vspace{1cm}\textit{[ΔΙΑΓΡΑΜΜΑ: Channel Manager Architecture]}\\\small Περιγραφή: Διάγραμμα που δείχνει τον Channel Manager στο κέντρο, με βέλη προς τα OTAs (Airbnb, Booking.com, Expedia, Vrbo) στη μία πλευρά και βέλη προς τα Properties/PMS στην άλλη. Ετικέτες: «Rates \& Availability Sync», «Booking Notifications», «Two-way Communication».\vspace{1cm}}}
