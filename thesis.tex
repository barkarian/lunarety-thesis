...
\begin{lstlisting}[caption={Παράδειγμα CollectionConfig με custom components και conditional logic}]
...
export const Bookings: CollectionConfig = {
  ...
  // Lifecycle hooks
  hooks: {
    beforeChange: [calculateCommission, syncToChannelManager],
    afterChange: [sendNotifications, updateRelatedRates],
  },
};
\end{lstlisting}
...
\begin{itemize}
    ...
    \item Το \code{filterOptions} φιλτράρει δυναμικά τις επιλογές ενός relationship field.
    \item Τα \code{hooks} ορίζουν τη business logic που εκτελείται κατά τον κύκλο ζωής του document.
\end{itemize}

\subsubsection{Relations και Joins}
...
\begin{lstlisting}[caption={Παράδειγμα Relations με filterOptions και where clauses}]
// Property Collection - One-to-Many με Bookings
  ...
  fields: [
    {
      ...
      relationTo: 'users',
      required: true,
      // Filter: Show only users of type 'manager'
      filterOptions: {
        type: { equals: 'manager' },
      },
    },
    {
      ...
      type: 'relationship',
      relationTo: 'users',
      // Dynamic filter based on the selected manager
      filterOptions: ({ siblingData }) => ({
        type: { equals: 'owner' },
        managedBy: { equals: siblingData.manager },
      }),
    },
    // Virtual/Join field - reverse lookup
    {
      name: 'bookings',
      type: 'join',
      collection: 'bookings',
      on: 'property', // Field in bookings that references this property
      where: {
...
// Querying with depth and where clauses
const properties = await payload.find({
  collection: 'properties',
  where: {
    manager: { equals: currentUser.id },
    'owner.plan.planCommission.commissionType': { equals: 'percentage' },
  },
